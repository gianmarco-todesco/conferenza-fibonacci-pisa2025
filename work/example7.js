...
window.addEventListener("DOMContentLoaded", () => {
    ...
    board = new Board(20, 3*7, 3*12);
    board.group.translation.set(two.width / 2, two.height / 2);
...
document.addEventListener("keydown", (event) => {
    if(event.key === "ArrowRight") {
        if(currentBoard) {
            currentBoard.split();
            currentBoard = currentBoard.children[0];
        }
    } else if(event.key === "r") {
        gsap.to(currentBoard, {bigGridVisible: 1, duration: 1} );
    }
});